...
|---|---|---|---|
| **`compactq-check` unitary-witness path — EXPERIMENTAL, top priority fix** | The checker's dense re-computation uses a q0=MSB wire convention that diverges from qiskit's q0=LSB on multi-qubit prefixes (symptom: fidelity 0.46 on qft(4) instead of 1.0). Per-gate matrices are verified correct; the bug is in the multi-wire bit embedding in `apply_dense`. Stabilizer + phase-polynomial paths are convention-free and correct (verified against QCEC/Qiskit). Fix = make `apply_dense` little-endian (`shifts = ws`), re-run mutation suite | the independent checker is the moat — it must be perfect, not merely present | next session: fix embedding, re-run `tests/test_checker_independence.py` (4 groups), then promote from EXPERIMENTAL |
